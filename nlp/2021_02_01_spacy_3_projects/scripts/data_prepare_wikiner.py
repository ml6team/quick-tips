...
def main(data_loc: Path, out_dir: Path, fraction: float = 0.2, seed: int = 0):
    """Partition the data into train/dev/test split."""
    random.seed(0)
    lines = (
    ...
    test_size = int(fraction * len(lines))
    train_size = (len(lines) - dev_size) - test_size
    if out_dir.exists():
        shutil.rmtree(out_dir)

    out_dir.mkdir(parents=True)
    with (out_dir / "wikiner_train.iob").open("w", encoding="utf8") as file_:
        file_.write("\n".join(lines[:train_size]))
    with (out_dir / "wikiner_dev.iob").open("w", encoding="utf8") as file_:
        file_.write("\n".join(lines[train_size: train_size + dev_size]))
    with (out_dir / "wikiner_test.iob").open("w", encoding="utf8") as file_:
